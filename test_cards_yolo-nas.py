...
model_path = '/home/noy/YOLO-NAS/PII/checkpoints/yolo_nas_s_experiment/RUN_20241013_065007_105865/average_model.pth'
    ...
    with open(annotation_path, 'w') as f:
        ...
        for result in processed_predictions:
            ...
            if score > threshold:
                ...
                cv2.rectangle(resized_image, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 0), -1)

                # Ensure the class_id is within the range of class_names
                if 0 <= class_id < len(class_names):
                    class_name = class_names[int(class_id)].upper()
                    #cv2.putText(resized_image, class_name, (int(x1), int(y1 - 10)),
                    ...
                    if class_name.lower() in class_names:
                        ...
                        resized_image[int(y1):int(y2), int(x1):int(x2)] = 0
...
print(f"YOLO-NAS runtime: {elapsed_time} seconds")